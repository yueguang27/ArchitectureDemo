 ...
 */
public class SimpleWorker5 extends Worker {
    @NonNull
    @Override
    public Result doWork() {
        Log.i(CONSTANT.TAG_WORKER, "SimpleWorker5_doWork_start----");

        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("时间", Utils.getTimeFormat(System.currentTimeMillis(), "yyMMdd HH:mm:ss SSS") + "--SimpleWorker5");
            Utils.writePathContent(CONSTANT.PATH, "\r" + jsonObject.toString());
            Log.i(CONSTANT.TAG_WORKER, "jsonObject:" + "," + jsonObject.toString());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        Log.i(CONSTANT.TAG_WORKER, "SimpleWorker5_doWork_end----");
